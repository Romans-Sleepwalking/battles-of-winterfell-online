...
import Host.Rooms.{Player, Room}
import akka.actor.{ActorRef, ActorSystem, PoisonPill, Props}
import com.corundumstudio.socketio.listener.{ConnectListener, DataListener, DisconnectListener}
import com.corundumstudio.socketio.{AckRequest, Configuration, SocketIOClient, SocketIOServer}
import play.api.libs.json.JsValue
import Host.{SignUp, Login, Fight, Action}


class Server() {

  class SignUpListener(server: Server) extends DataListener[JsValue] {
    override def onData(socket: SocketIOClient, regData: JsValue, ackRequest: AckRequest): Unit = {
      ...
      actor ! SignUp(regData, server)
    }
  }


  class LoginListener(server: Server) extends DataListener[JsValue] {
    override def onData(socket: SocketIOClient, logData: JsValue, ackRequest: AckRequest): Unit = {
      println(socket + " trying to login!")
      val actor: ActorRef = server.Socket_2_Actor(socket)
      actor ! Login(logData, socket, server)
    }
  }


  class FightListener(server: Server) extends DataListener[JsValue] {
